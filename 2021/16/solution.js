...
const parsed = parse(hexaToBinary(actualInput));

const getValue = item => {
  if (item.value) {
    return item.value;
  } else {
    const values = item.content.map(getValue);
    return reduceRules[item.typeId](values)
  }
}

const reduceRules = {
  0: _.sum,
  1: vs => vs.reduce((a, b) => a * b),
  2: vs => Math.min(...vs),
  3: vs => Math.max(...vs),
  5: ([a, b]) => a > b ? 1 : 0,
  6: ([a, b]) => a < b ? 1 : 0,
